...
function computNextBoard(states: number, current: Board, next: Board) {
  const ALIVE = 1;
  const DEAD = 0;
  ...
  for (let r = 0; r < BOARD_ROWS; ++r) {
    for (let c = 0; c < BOARD_COLS; ++c) {
      ...
      switch (current[r][c]) {
        case DEAD:
          if (nbors[ALIVE] === 3) next[r][c] = ALIVE;
          else next[r][c] = DEAD;
          break;
        case ALIVE:
          if (nbors[ALIVE] === 2 || nbors[ALIVE] === 3) next[r][c] = ALIVE;
          else next[r][c] = DEAD;
          break;
      }
    }
...
/** renders the cell */
function render(ctx: CanvasRenderingContext2D, currentBoard: Board) {
...
if (next === null) throw new Error(`next ka button nahi mil raha`);
